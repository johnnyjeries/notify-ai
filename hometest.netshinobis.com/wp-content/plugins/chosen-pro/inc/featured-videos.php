...
function ct_chosen_pro_video_callback( $post ) {
	...
	$youtube_mute     = get_post_meta( $post->ID, 'ct_chosen_pro_video_youtube_mute', true );

	// video preview
	$container_classes = 'ct_chosen_pro_video_preview_container';
	if ( strpos( $video_url, 'youtube-nocookie.com' ) !== false ) {
		$container_classes .= ' youtube-nocookie';
	}
	echo '<div class="'. esc_attr($container_classes) .'" id="ct_chosen_pro_video_preview_container">';
	...
	echo '<div class="ct_chosen_pro_video_youtube_controls_container ' . esc_attr( $class ) . '">';
		...
		echo '</label> ';
		echo '<label for="ct_chosen_pro_video_youtube_mute">';
			echo '<input type="checkbox" name="ct_chosen_pro_video_youtube_mute" id="ct_chosen_pro_video_youtube_mute" value="1" ' . checked( '1', $youtube_mute, false ) . '>';
			esc_html_e( 'Auto-mute video', 'chosen-pro' );
		echo '</label> ';
	echo '</div>';
}
...
function ct_chosen_pro_output_video( $video_url ) {
	...
	if ( $video_url ) {
		if ( strpos( $video_url, 'youtube-nocookie.com' ) !== false ) {
			return '<iframe src="'. esc_url( $video_url ) .'" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>';
		}
...
function ct_chosen_pro_video_save_data( $post_id ) {
	...
	$youtube_IDs = array(
		...
		'ct_chosen_pro_video_youtube_mute'
	);
...
function ct_chosen_pro_output_featured_video( $featured_image ) {
	...
	if ( $featured_video ) {
		...
		if (
			...
			$featured_image = '<div class="'. esc_attr($container_classes) .'">' . ct_chosen_pro_output_video( $featured_video ) . '</div>';

			// If AMP plugin is active and viewing an AMP page
			if ( function_exists('amp_init') ) {
				if ( is_amp_endpoint() ) {
					// Add filter to remove the Featured Image
					add_filter( 'amp_post_template_data', 'ct_chosen_pro_amp_remove_featured_image' );
				}
			}
		}
	}
...
function ct_chosen_pro_featured_video_amp( $content ) {
	...
	if ( function_exists('amp_init') ) {
		if ( is_amp_endpoint() ) {
			...
			global $post;
			$featured_video = ct_chosen_pro_output_featured_video('');
...
add_filter( 'the_content', 'ct_chosen_pro_featured_video_amp', 1 );

function ct_chosen_pro_amp_remove_featured_image( $data ) {
