	...
	<div class="license-container">
		<form method="post" action="options.php">
			...
			<?php if( $status !== false && $status == 'valid' ) : ?>
				<p><?php printf( __('Your license key has been activated! You can now update Chosen Pro automatically through the <a href="%s">Updates</a> menu.', 'chosen-pro'), esc_url( $updates_url ) ); ?></p>
			<?php else : ?>
			...
			<table class="form-table">
				...
				<tr valign="top" class="license-key">
					...
					<td>
						...
						<?php endif;?>
						<input id="ct_chosen_pro_license_key" name="ct_chosen_pro_license_key" type="text" class="regular-text" value="<?php echo esc_attr( $license ); ?>" placeholder="c39b426c0c9bcd6a41f5a450e513de10" />
					</td>
				</tr>
				<tr valign="top" class="activate-deactivate">
					<?php if( $status !== false && $status == 'valid' ) { ?>
						<th scope="row" valign="top">
							<?php esc_html_e('Deactivate License Key','chosen-pro'); ?>
						</th>
						<td>
					...
					<?php } else { ?>
						<th scope="row" valign="top">
							<?php esc_html_e('Activate License Key','chosen-pro'); ?>
						</th>
						<td>
...
function ct_chosen_pro_plugin_updater() {
	...
	$edd_updater = new EDD_SL_Plugin_Updater( EDD_SL_STORE_URL, CHOSEN_PRO_FILE, array(
			'version'   => '1.24',        // current version number
...
function ct_chosen_pro_activate_license() {
	...
	if( isset( $_POST['ct_chosen_pro_license_activate'] ) ) {
		...
		if ( isset( $_POST['ct_chosen_pro_license_key'] ) ) {
			$license = trim( $_POST['ct_chosen_pro_license_key'] );		
		} else {
		...
		update_option( 'ct_chosen_pro_license_key', $license );
